...
Read this **after** `PROJECT_STATE.md` §1. TL;DR: everything below is **shipped, merged to
...
- **Desk vs laptop:** the **live feed** (`I:\Public\Summer Interns Energy\DB\`) is only visible from
  ...
  gitignored (PROJECT_STATE.md §5) — the laptop needs them restored to run the dashboard or research legs.
...
> Read `PROJECT_STATE.md` and `docs/SESSION_HANDOFF_2026-06-22.md` in pulse/ first. I'm continuing the
...
> not have `/Data`/model pkls (PROJECT_STATE.md §5) — so favour code/docs tasks unless I've restored those.
...
> Don't multi-task; do the one thing, update PROJECT_STATE.md + docs, and stop.
